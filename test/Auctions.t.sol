...
contract AuctionsTest is Test {
    ...
    uint256[] public tokenIds2 = [1, 2, 3];
    ...
    // startAuctionERC721()
    //
    function test_startAuctionERC721_doesnot_take_token_ids_in_account_while_checking_minStartingBid() public {
        vm.startPrank(user1);

        uint256 startBalance = user1.balance;
        assertEq(bidTicket.balanceOf(user1, 1), 100);

        auctions.startAuctionERC721{value: 0.05 ether}(0.05 ether, address(mock721), tokenIds1);

        auctions.startAuctionERC721{value: 0.05 ether}(0.05 ether, address(mock721), tokenIds2);

        (, address tokenAddress,,uint8 tokenCount1,, address highestBidder, uint256 highestBid,,) = auctions.auctions(1);

        console.log("Auction 1", tokenCount1, highestBidder, highestBid);

        (, address tokenAddress2,,uint8 tokenCount2,, address highestBidder2, uint256 highestBid2,,) = auctions.auctions(2);

        console.log("Auction 2", tokenCount2, highestBidder2, highestBid2);
